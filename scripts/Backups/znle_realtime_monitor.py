#!/usr/bin/env python3
"""
Real-Time ZNLE6 Signal Monitor with Live Plot
...
alignment and signal tracking applications.

Usage:
  python3 znle_realtime_monitor.py --ip 192.168.15.90 --freq 977e6 --param S21
...
class RealtimeMonitor:
    """Real-time VNA monitor with live plotting"""
    ...
    def __init__(self, ip, port, freq, param, csv_path, max_points=1000, smoothing_window=10):
        ...
        self.ax.set_title(f'Real-Time Signal Monitor - {param} @ {freq/1e6:.1f} MHz{smooth_text}', 
        ...
        if amplitude is not None:
            ...
            if len(self.times) > 0:
                ...
                if len(self.amplitudes) > 0:
                    ...
                    amp_range = np.ptp(amp_array) if len(amp_array) > 1 else 10
                    margin = max(5, amp_range)
                    self.ax.set_ylim(amp_mean - margin, amp_mean + margin)
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Real-time ZNLE6 signal monitor with live plotting",
    ...
    ap.add_argument("--filename", default="realtime_monitor",
                   help="Base filename for CSV output (default: realtime_monitor)")
    ...
    print("="*60)
    print("ZNLE6 Real-Time Signal Monitor")
    print("="*60)
